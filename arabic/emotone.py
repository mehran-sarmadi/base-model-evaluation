...
def emotone_main(model_name_or_path, logger, output_dir, tokenizer_name_or_path=None):
    logger.info("Loading dataset...")
    raw_datasets = load_dataset("emotone-ar-cicling2017/emotone_ar")
    train_val_split = raw_datasets['train'].train_test_split(test_size=0.3, seed=42)  
    val_test_split = train_val_split['test'].train_test_split(test_size=0.75, seed=42)
    raw_datasets['train'] = train_val_split['train']
    raw_datasets['validation'] = val_test_split['train']
    raw_datasets['test'] = val_test_split['test']
    ...
    def preprocess_function(examples):
        ...
        result = tokenizer(examples["tweet"], max_length=512, truncation=True)
    ...
    training_args = TrainingArguments(
        output_dir=f"output_dir/{'emoton'}/",
        save_total_limit=2,
        learning_rate=5e-5,
